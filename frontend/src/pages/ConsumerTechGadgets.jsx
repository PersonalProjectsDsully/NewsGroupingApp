// src/pages/ConsumerTechGadgets.jsx
import React, { useState, useEffect } from 'react';
import { ChevronDown, ChevronRight, Clock, Loader2, Smartphone } from 'lucide-react';
import Card from '../components/card'; // Assuming Card component exists

export default function ConsumerTechGadgets({ darkMode, selectedTimeRange }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`/api/consumer_tech_gadgets_groups?hours=${selectedTimeRange}`);
        if (!res.ok) {
        ...
        console.error('Error fetching Consumer Tech groups:', error);
      ...
      {!isLoading && groups.length === 0 && (
        <div className="text-center py-10">
          <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4 ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
            <Smartphone className={`w-8 h-8 ${darkMode ? 'text-gray-300' : 'text-gray-500'}`} />
          </div>
          <h3 className="text-xl font-medium mb-2">No consumer tech updates</h3>
          <p className={`text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6`}>
            {selectedTimeRange > 0
              ? `No consumer tech articles found in the last ${selectedTimeRange} hour${selectedTimeRange !== 1 ? 's' : ''}.`
              : 'No consumer tech articles found.'}
          </p>
          {selectedTimeRange > 0 && (
            <button
