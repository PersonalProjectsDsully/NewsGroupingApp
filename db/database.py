...
def _execute_write(sql, params, db_path, cursor=None):
    ...
    try:
        ...
        if "locked" in str(e).lower():
             logger.error(f"DATABASE LOCKED executing query. SQL: {sql[:100]}... Params: {params}")
        else:
             logger.error(f"Database error executing query: {e}. SQL: {sql[:100]}... Params: {params}", exc_info=True)
        if conn_managed_here and conn:
            conn.rollback()
...
def insert_entity(entity_name, entity_type, description=None, db_path="db/news.db", cursor=None):
    ...
    try:
        ...
        cursor.execute("SELECT entity_id FROM entity_profiles WHERE entity_name = ? AND entity_type = ?", (entity_name, entity_type))
        ...
        logger.error(f"Error in insert_entity for '{entity_name}': {e}", exc_info=False) # Less verbose logging maybe
...
def link_entity_to_article(article_id, entity_id, relevance_score=1.0, context_snippet=None, db_path="db/news.db", cursor=None):
...
def link_entity_to_group(group_id, entity_id, relevance_score=1.0, db_path="db/news.db", cursor=None):
...
def link_entity_to_trend(trend_id, entity_id, relevance_score=1.0, db_path="db/news.db", cursor=None):
...
def create_exemplar(category, pattern_name, pattern_description=None, success_score=1.0, db_path="db/news.db", cursor=None):
...
def add_article_to_exemplar(exemplar_id, article_id, db_path="db/news.db", cursor=None):
...
def insert_article_cve(article_id, cve_id, published_date, db_path="db/news.db", cursor=None):
...
def insert_or_update_cve_info(cve_id, base_score, vendor, affected_products, cve_url, vendor_link, solution, times_mentioned, raw_json_str, db_path="db/news.db", cursor=None):
    ...
    params = (
        cve_id, base_score, vendor, affected_products, cve_url,
        vendor_link, solution, times_mentioned, raw_json_str
    )
    try:
        _execute_write(sql, params, db_path, cursor)
        return True
