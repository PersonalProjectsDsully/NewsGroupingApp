...
from db.database import get_connection, setup_database
from utils import approximate_tokens # Assuming this exists
...
try:
    ...
    from analysis.consistency_checker import evaluate_group_consistency
...
PREDEFINED_CATEGORIES = [
    "Science & Environment", "Business, Finance & Trade",
    "Artificial Intelligence & Machine Learning", "Software Development & Open Source",
    "Cybersecurity & Data Privacy", "Politics & Government",
    "Consumer Technology & Gadgets", "Automotive, Space & Transportation",
    "Enterprise Technology & Cloud Computing", "Other"
]

# --- Configuration ---
# LLM Check Configuration
ENABLE_LLM_MATCH_ASSESSMENT = True # Set to False to disable LLM checks for ambiguous cases
...
DYNAMIC_THRESHOLD_RULES = {
    ...
    'size_adjust': { # Additive adjustments based on article count in group
        ...
        'adjustments': [+0.05, 0.0, -0.03, -0.05] # Adjustments for <=1, <=5, <=10, >10
    }
}
...
def create_new_group_for_single_article(
    ...
    if closest_groups_info:
        ...
        for i, group_info in enumerate(closest_groups_info[:2]): # Show top 2 near misses
    ...
    messages = [
        {"role": "system", "content": "You are an expert news analyst. Define a new group based on the provided article, considering the context of near-miss groups if provided. Respond only in JSON."},
        {"role": "user", "content": prompt}
    ]

    response = call_gpt_api(messages, api_key, model=LLM_CHECK_MODEL) # Use configured model
    if not response:
        logger.error(f"LLM call failed for creating new group for article {article_id}.")
        return None
    ...
    try:
        ...
        if main_topic not in PREDEFINED_CATEGORIES: main_topic = "Other"
    except json.JSONDecodeError as e:
        logger.error(f"Failed to parse LLM JSON for article {article_id}: {e}\nResponse: {cleaned}")
        main_topic, group_label, description = "Other", f"Group for Article {article_id} (Auto)", article_title
    ...
    try:
        ...
        logger.info(f"Created new group (ID: {new_group_id}, Label: '{group_label}') for article {article_id}.")

        # Use the existing cursor for add_article_to_group
        success = add_article_to_group(article_id, new_group_id, db_path=db_path, cursor=cursor)

        if not success:
             logger.error(f"Failed to add article {article_id} to its newly created group {new_group_id}")
             conn.rollback()
             return None
        ...
        new_group_dict = {
             "group_id": new_group_id, "main_topic": main_topic, "sub_topic": "",
             "group_label": group_label, "description": description,
             "consistency_score": 0.7, "article_ids": [article_id]
             # Include other fields needed for signature generation if different from DB defaults
        }
        return new_group_dict

    except sqlite3.Error as e:
        logger.error(f"Database error creating new group for article {article_id}: {e}", exc_info=True)
...
def _calculate_dynamic_threshold(group_dict: Dict, base_threshold: float, rules: Dict) -> float:
...
def _get_group_details_for_prompt(group_id: int, groups_with_signatures: List[Tuple[Dict, Dict]]) -> Optional[Dict]:
     ...
     for group_dict, group_sig in groups_with_signatures:
          if group_dict.get("group_id") == group_id:
               ...
               key_entities = [e.get("entity_name") for e in group_sig.get("primary_entities", [])[:5] if e.get("entity_name")]
               return {
                    "id": group_id,
...
def process_single_ungrouped_article(
    ...
    article_title: str,
    article_content: str,
    existing_groups_with_signatures: List[Tuple[Dict, Dict]], # List of [group_dict, group_signature]
    ...
    try:
        ...
        if not article_sig:
            logger.warning(f"Could not generate signature for article {article_id}. Skipping.")
            return {"status": "error", "message": "Signature generation failed", "article_id": article_id}
        ...
        for group_data in existing_groups_with_signatures:
            ...
            if not group_sig or not group_dict.get("article_ids"): continue

            # Calculate dynamic threshold for *this specific group*
            current_dynamic_threshold = _calculate_dynamic_threshold(group_dict, base_threshold, threshold_rules)

            # Calculate similarity (ASSUMES THIS FUNCTION IS UPDATED)
            # Pass api_key if calculate_... needs it for description similarity
            similarity_scores = calculate_article_to_group_similarity(article_sig, group_sig, api_key=api_key)
        ...
        logger.debug(f"Article {article_id}: Best match Group {best_match_group['group_id'] if best_match_group else 'None'} (Score: {best_match_score:.3f}, DynThr: {best_match_threshold_used:.3f}, 2ndBest: {second_best_score:.3f})")
        ...
        if best_match_group:
            ...
            is_in_ambiguity_zone = (
                (best_match_threshold_used - AMBIGUITY_ZONE_BELOW_THRESHOLD <= best_match_score < best_match_threshold_used + AMBIGUITY_ZONE_ABOVE_THRESHOLD)
                or (is_match_above_threshold and best_match_score - second_best_score < MAX_SCORE_GAP_FOR_AMBIGUITY)
            )
            ...
            # Condition 2: Ambiguous case - potentially use LLM
                 ...
                 logger.info(f"Article {article_id}: Ambiguous match score ({best_match_score:.3f} vs Thr {best_match_threshold_used:.3f}). Triggering LLM check.")

                 # Prepare context for LLM
                 article_entities = [e.get("entity_name") for e in article_sig.get("primary_entities", [])[:10] if e.get("entity_name")]
                 candidate_group_ids = [g["group_id"] for g in group_scores[:3]] # Top 3 candidates
                 candidate_groups_details = [_get_group_details_for_prompt(gid, existing_groups_with_signatures) for gid in candidate_group_ids]
                 candidate_groups_details = [d for d in candidate_groups_details if d] # Filter out None
                 ...
                 if candidate_groups_details:
                     ...
                     llm_decision_str = call_gpt_api(messages, api_key, model=LLM_CHECK_MODEL)
                     ...
                     if llm_decision_str:
                         try:
                             ...
                             if llm_group_id not in candidate_group_ids:
                                 logger.warning(f"LLM returned group ID {llm_group_id} which was not in candidates {candidate_group_ids}. Ignoring.")
                                 llm_group_id = None
                         except (ValueError, TypeError):
                             if "none" in llm_decision_str.lower():
                                 llm_group_id = "None"
                             else:
                                 logger.warning(f"LLM assessment returned unparsable result: '{llm_decision_str}'. Falling back.")
                     ...
                     if isinstance(llm_group_id, int):
                         logger.info(f"LLM Assessment: Match Article {article_id} to Group {llm_group_id}.")
                         decision = "add_to_existing"
                         final_group_id = llm_group_id
                     elif llm_group_id == "None":
                         logger.info(f"LLM Assessment: Article {article_id} does not match candidates. Create new group.")
                         decision = "create_new"
                     else: # LLM failed or unclear response
                         logger.info("LLM Assessment failed or unclear. Falling back to threshold logic.")
                     ...
                     logger.warning(f"Could not get candidate group details for LLM check for article {article_id}. Falling back.")
                     # Fallback: Use the original threshold comparison
                     if is_match_above_threshold:
                          decision = "add_to_existing"
                          final_group_id = best_match_group["group_id"]
        ...
        if decision == "add_to_existing":
            logger.info(f"Decision: Add Article {article_id} to Group {final_group_id} (Score: {best_match_score:.3f}, Thr: {best_match_threshold_used:.3f}, LLM Checked: {llm_check_triggered})")
            ...
            try:
                ...
                success = add_article_to_group(article_id, final_group_id, db_path=db_path, cursor=cursor_add)
                if success:
                    conn_add.commit()
                else:
                    conn_add.rollback()
            except Exception as add_err:
                 logger.error(f"Error during add_article_to_group transaction for article {article_id}: {add_err}")
                 if conn_add: conn_add.rollback()
                 success = False
            finally:
                 if conn_add: conn_add.close()
            ...
            if success:
                ...
                for i, (grp_d, grp_s) in enumerate(existing_groups_with_signatures):
                    if grp_d["group_id"] == final_group_id:
                        existing_groups_with_signatures[i][0]["article_ids"].append(article_id)
                        # Re-generating signature here is too costly for a long run.
                        break

                # --- Optional: Post-Add Consistency Check ---
                try:
                    updated_group_data_dict = next((g[0] for g in existing_groups_with_signatures if g[0]["group_id"] == final_group_id), None)
                    if updated_group_data_dict and len(updated_group_data_dict.get("article_ids", [])) > 1:
                        evaluation = evaluate_group_consistency(updated_group_data_dict, api_key, db_path)
                        consistency_score = evaluation.get("consistency_score", 0.5)
                        logger.info(f"Consistency check for Group {final_group_id} after adding Article {article_id}: Score={consistency_score:.2f}")
                    ...
                    logger.error(f"Error during post-add consistency check: {cons_err}", exc_info=False)
                # --- End Consistency Check ---

                return {"status": "added_to_existing", "article_id": article_id, "group_id": final_group_id, "group_label": best_match_group["group_label"], "score": best_match_score}
            else:
                 return {"status": "error", "message": f"Failed to add article {article_id} to group {final_group_id}", "article_id": article_id}

        elif decision == "create_new":
            logger.info(f"Decision: Create new group for Article {article_id} (Best score: {best_match_score:.3f} vs Thr: {best_match_threshold_used:.3f}, LLM Checked: {llm_check_triggered})")
            # Prepare context of near misses for the LLM
            near_miss_groups = []
            if group_scores:
                 near_miss_groups = [
                      {'label': g['group_label'], 'desc': g['description'], 'score': g['score']}
                      for g in group_scores[:2] # Pass top 2 regardless of score
                 ]

            new_group_info = create_new_group_for_single_article(
                article_id, article_title, article_content, api_key, db_path,
                closest_groups_info=near_miss_groups # Pass near-miss context
            )
            if new_group_info:
                return {"status": "created_new", "article_id": article_id, "new_group_info": new_group_info}
            else:
                return {"status": "error", "message": f"Failed to create new group for article {article_id}", "article_id": article_id}

        else: # Should not happen
             logger.error(f"Invalid decision state '{decision}' for article {article_id}")
             return {"status": "error", "message": "Internal error in decision logic", "article_id": article_id}
...
def run_grouping_update(
    ...
    if api_key is None: api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        logger.error("API Key not found. LLM checks and new group creation will fail. Aborting grouping.")
        return
    ...
    for group in existing_groups:
        ...
        if group.get("article_ids"):
            try:
                 group_sig = generate_group_signature(group, db_path) # Assumes updated version
                 if group_sig:
                      existing_groups_with_signatures.append([group, group_sig]) # Use list for mutability
                 else:
                      logger.warning(f"Could not generate signature for group {group.get('group_id')}")
            except Exception as sig_err:
                 logger.error(f"Error generating signature for group {group.get('group_id')}: {sig_err}", exc_info=False)
    logger.info(f"Finished generating initial signatures for {len(existing_groups_with_signatures)} groups.")
    ...
    for index, row in ungrouped_df.iterrows():
        ...
        logger.info(f"Processing article {index + 1}/{total_articles} (ID: {article_id})")
        ...
        if result["status"] == "added_to_existing":
            ...
            if new_group_info:
                logger.info(f"Generating signature for newly created group {new_group_info.get('group_id')}...")
                try:
                    new_group_sig = generate_group_signature(new_group_info, db_path)
                    if new_group_sig:
                        existing_groups_with_signatures.append([new_group_info, new_group_sig]) # Append mutable list
                        logger.info(f"Added new group {new_group_info.get('group_id')} to comparison list ({len(existing_groups_with_signatures)} total).")
                    else:
                         logger.warning(f"Failed to generate signature for new group {new_group_info.get('group_id')}. It won't be used for matching in this run.")
                except Exception as new_sig_err:
                     logger.error(f"Error generating signature for new group {new_group_info.get('group_id')}: {new_sig_err}", exc_info=False)
            else:
                 logger.error("Processing status was 'created_new' but no group info was returned.")
                 results_summary["errors"] += 1
        elif result["status"] == "error":
            results_summary["errors"] += 1
            logger.error(f"Error processing article {article_id}: {result.get('message')}")
