...
def get_primary_entities(article_id: int, min_relevance: float = 0.7, db_path: str = "db/news.db") -> List[Dict]:
    """
...
def get_companies_for_article(article_id: int, db_path: str = "db/news.db") -> List[str]:
    """
...
def get_entities_by_type(article_id: int, entity_type: str, db_path: str = "db/news.db") -> List[Dict]:
    """
...
def generate_article_signature(article_id: int, db_path: str = "db/news.db") -> Dict[str, Any]:
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT published_date, source FROM articles WHERE id = ?",
            (article_id,)
        )
        row = cursor.fetchone()
        if row:
    ...
    primary_entities = get_primary_entities(article_id, min_relevance=0.7, db_path=db_path)
...
def calculate_signature_similarity(sig1: Dict, sig2: Dict) -> Dict[str, float]:
    ...
    if entity_ids1 and entity_ids2:
        ...
        results["entity_similarity"] = entity_intersection / entity_union if entity_union > 0 else 0.0
    else:
    ...
    if companies1 and companies2:
        ...
        results["company_similarity"] = company_intersection / company_union if company_union > 0 else 0.0
    else:
    ...
    if cves1 and cves2:
        ...
        results["cve_similarity"] = cve_intersection / cve_union if cve_union > 0 else 0.0
    else:
    ...
    if event_names1 and event_names2:
        ...
        results["event_similarity"] = event_intersection / event_union if event_union > 0 else 0.0
    else:
    ...
    if urls1 and urls2:
        ...
        results["reference_similarity"] = url_intersection / url_union if url_union > 0 else 0.0
    else:
        results["reference_similarity"] = 0.0
    
    # Author similarity
    results["author_similarity"] = 1.0 if sig1.get("author") == sig2.get("author") and sig1.get("author") is not None else 0.0
...
def find_candidate_matches(article_id: int, min_score: float = 0.4, limit: int = 10, db_path: str = "db/news.db") -> List[Dict]:
    """
...
def generate_signatures_for_all_articles(db_path: str = "db/news.db") -> Dict[int, Dict]:
    """
...
def create_signature_index(db_path: str = "db/news.db"):
    ...
    for i, article_id in enumerate(article_ids):
        ...
        entity_hash = hashlib.md5(json.dumps(sorted([e["entity_id"] for e in signature["primary_entities"]])).encode()).hexdigest()
        company_hash = hashlib.md5(json.dumps(sorted(signature["companies"])).encode()).hexdigest()
        cve_hash = hashlib.md5(json.dumps(sorted(signature["cves"])).encode()).hexdigest()
        event_hash = hashlib.md5(json.dumps(sorted([e["event_name"] for e in signature["events"]])).encode()).hexdigest()
        reference_hash = hashlib.md5(json.dumps(sorted([r["url"] for r in signature["references"]])).encode()).hexdigest()
...
def find_matches_by_entity_hash(entity_hash: str, db_path: str = "db/news.db") -> List[int]:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--create-index", action="store_true", help="Create signature index")
    parser.add_argument("--article-id", type=int, help="Generate signature for specific article")
    parser.add_argument("--find-matches", action="store_true", help="Find matches for article")
    ...
    if args.article_id:
        ...
        if args.find_matches:
            ...
            for match in matches:
                print(f"Article {match['article_id']}: {match['similarity']['composite_score']:.2f} similarity")
