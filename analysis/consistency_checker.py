...
from db.database import get_connection, create_exemplar, add_article_to_exemplar
...
from analysis.entity_extraction import get_entities_for_article
...
def evaluate_group_consistency(group_data, api_key, db_path="db/news.db"):
    ...
    historical_groups = get_recent_groups_for_category(category, days=30, db_path=db_path)
    ...
    if not response:
        logger.warning(f"No response from GPT for consistency evaluation of group '{group_label}'")
        return {
            "consistency_score": 0.5,  # Default neutral score
    ...
    try:
        ...
        if data.get("exemplar_worthy", False) and data.get("consistency_score", 0) >= 0.8:
            try:
                ...
                logger.info(f"Created exemplar {exemplar_id} for group with label '{group_label}'")
