        ...
        for entry in root.findall('atom:entry', ns):
            ...
            if content_elem is not None:
                if content_elem.get('type') == 'html':
                    soup = BeautifulSoup(content_elem.text or '', 'html.parser')  # handle None case
                    # Directly get text, removing any additional divs.
                    content_parts = []
                    for tag in soup.find_all(['p', 'blockquote'], recursive=False): #check all available tags.
                        # Exclude specific classes like 'entry-tags' and 'posted'
                        if not any(cls in (tag.get('class') or []) for cls in ['entry-tags', 'posted']): #class check
                            content_parts.append(tag.get_text().strip())
                    content = '\n'.join(filter(None, content_parts))  # filter out empty strings
                else:
                    content = content_elem.text or ''  # Fallback to raw text if not HTML
