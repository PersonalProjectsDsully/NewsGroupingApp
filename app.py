...
from analysis.trending_analysis import get_trending_topics, cleanup_old_trends, ensure_minimum_trends
from analysis.entity_extraction import get_trending_entities, get_entities_for_category
from pipeline import schedule_regular_cleanup
...
def fetch_groups_for_category(category_value, hours=None):
    ...
    if hours is not None and hours > 0:
        ...
        logger.info(f"Filtering articles for category '{category_value}' published after: {cutoff_iso}")
    else:
         logger.info(f"Fetching all articles for category '{category_value}' (no time filter).")

    conn = get_connection()
    c = conn.cursor()
    groups_data = []
    try:
        c.execute("SELECT group_id, main_topic, group_label, description FROM two_phase_article_groups WHERE main_topic = ?", (category_value,))
        ...
        for (group_id, _, group_label, description) in group_definitions:
            ...
            if not article_rows: continue

            articles_list = []
            latest_article_date = None
            for (article_id, link, title, pubdate, content) in article_rows:
                preview = (content or "")[:300] + "..." if content else ""
                articles_list.append({"article_id": article_id, "link": link, "title": title, "published_date": pubdate, "preview": preview})
                if pubdate and (latest_article_date is None or pubdate > latest_article_date):
                    latest_article_date = pubdate

            groups_data.append({"group_id": group_id, "group_label": group_label, "description": description, "article_count": len(article_rows), "articles": articles_list, "latest_article_date": latest_article_date})
    except sqlite3.Error as e:
        logger.error(f"Error fetching groups/articles for category '{category_value}': {e}", exc_info=True)
    finally:
        conn.close()

    groups_data.sort(key=lambda x: (x["article_count"], x.get("latest_article_date", "")), reverse=True)
    for group in groups_data: group.pop("latest_article_date", None)
    logger.info(f"Found {len(groups_data)} groups with articles for category '{category_value}' in the time window.")
    return groups_data
...
def get_home_groups():
    ...
    categories_data = []
    try:
        from analysis.two_phase_grouping import PREDEFINED_CATEGORIES
    except ImportError:
...
def get_category_groups():
    ...
    hours = request.args.get("hours", type=int, default=None)
    logger.info(f"API call to /api/category_groups with category={category}, hours={hours}")
...
def get_ai_machine_learning_groups():
    hours = request.args.get("hours", type=int, default=None)
    groups = fetch_groups_for_category("Artificial Intelligence & Machine Learning", hours=hours)
    return jsonify({"category": "Artificial Intelligence & Machine Learning", "groups": groups})
...
def get_automotive_space_transportation_groups():
    hours = request.args.get("hours", type=int, default=None)
    groups = fetch_groups_for_category("Automotive, Space & Transportation", hours=hours)
...
def get_enterprise_cloud_computing_groups():
    hours = request.args.get("hours", type=int, default=None)
    groups = fetch_groups_for_category("Enterprise Technology & Cloud Computing", hours=hours)
    return jsonify({"category": "Enterprise Technology & Cloud Computing", "groups": groups})
...
def get_trending_api():
    ...
    logger.info(f"API call to /api/trending with category={category}, limit={limit}, hours={hours}")
    try:
        ...
        if hours and hours > 0:
            cutoff_time_iso = (datetime.now(pytz.UTC) - timedelta(hours=hours)).isoformat()
            logger.info(f"Filtering trending articles published after: {cutoff_time_iso}")
            for idx in df.index:
                articles = df.loc[idx, 'articles']
                if isinstance(articles, list):
                     filtered_articles = [
                         a for a in articles
                         if a.get('published_date') and str(a['published_date']) >= cutoff_time_iso
                     ]
                     df.loc[idx, 'articles'] = filtered_articles
                else:
                     df.loc[idx, 'articles'] = []
            df = df[df['articles'].apply(lambda x: isinstance(x, list) and len(x) > 0)].copy()
            ...
            for article in record.get("articles", []):
                ...
                if pub_date:
                    try:
                        ...
                        if pd.notna(dt): article["published_date"] = dt.isoformat()
                        else: article["published_date"] = str(pub_date) if isinstance(pub_date, str) else None
                    except Exception as e:
                         logger.warning(f"Could not format date {pub_date} for trending article {article.get('article_id')}: {e}")
                         article["published_date"] = str(pub_date) if isinstance(pub_date, str) else None
        return jsonify(records)
    except Exception as e:
...
def get_category_entities_api():
    ...
    if not category:
        return jsonify({"error": "Missing 'category' parameter"}), 400
    limit = request.args.get("limit", type=int, default=20)
    logger.info(f"API call to /api/category_entities with category={category}, limit={limit}")
    try:
        df = get_entities_for_category(category, limit=limit, db_path=DB_PATH)
        records = df.to_dict(orient="records")
        return jsonify(records)
    except Exception as e:
        logger.error(f"Error fetching category entities for '{category}': {e}", exc_info=True)
        return jsonify({"error": f"Failed to generate entities for category {category}"}), 500

@app.route("/api/prompt_tester/articles", methods=["GET"])
def get_recent_articles_for_testing():
    hours = request.args.get("hours", type=int, default=24)
    limit = request.args.get("limit", type=int, default=20)
    logger.info(f"API call to /api/prompt_tester/articles with hours={hours}, limit={limit}")
    cutoff_time = (datetime.now(pytz.UTC) - timedelta(hours=hours)).strftime("%Y-%m-%d %H:%M:%S")
    conn = get_connection()
    articles = []
    try:
        ...
        for row in rows:
             articles.append({
                 ...
                 "full_content": row[2], "published_date": row[3], "source": row[4]
             })
    except Exception as e:
...
def test_grouping_prompt():
    ...
    try:
        ...
        if df.empty: return jsonify({"error": "No articles found for the given IDs"}), 404

        articles_text = ""
        for _, row in df.iterrows():
            articles_text += f"Article ID={row['id']}:\nTitle: {row['title']}\n{row['content']}\n\n"
        ...
        api_key = os.environ.get('OPENAI_API_KEY')
        if not api_key: return jsonify({"error": "OpenAI API key not configured on server"}), 500

        response = call_gpt_api(messages, api_key, model=model)
        if response is None: return jsonify({"error": "Failed to get response from LLM API"}), 500
...
def debug_date_format():
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT id, published_date FROM articles ORDER BY published_date DESC LIMIT 30")
...
def serve_react_app(path):
    ...
    if os.path.exists(requested_path) and os.path.isfile(requested_path):
        ...
        logger.debug(f"Path '{path}' not found as static file, serving index.html for client-side routing.")
        return send_from_directory(app.static_folder, 'index.html')
