...
def _calculate_group_similarity(group_sig_a: Dict, group_sig_b: Dict, api_key: Optional[str]) -> float:
    ...
    sim_a_b_results = calculate_article_to_group_similarity(group_sig_a, group_sig_b, api_key)
    sim_b_a_results = calculate_article_to_group_similarity(group_sig_b, group_sig_a, api_key)
    ...
    logger.debug(f"Group Sim {group_sig_a.get('group_id')} vs {group_sig_b.get('group_id')}: AvgSigSim={avg_sim:.3f}, LLMLabelSim={label_desc_sim:.3f} -> Final={final_sim:.3f}")
    return final_sim


def merge_similar_groups(merge_threshold: float, api_key: str, db_path: str = "db/news.db") -> Dict:
    """
    ...
    """
    if not all([get_existing_groups_enhanced, generate_group_signature, calculate_article_to_group_similarity]):
         logger.error("Missing necessary functions from enhanced_grouping. Cannot perform merge.")
    ...
    logger.info(f"Generating signatures for {len(groups)} groups for merging analysis...")
    ...
    for group in groups:
         if group.get("article_ids"): # Only consider groups with articles
              ...
              if group_sig:
                  ...
                  logger.warning(f"Could not generate signature for group {group.get('group_id')} during merge prep.")
    ...
    for i in range(len(groups_with_signatures)):
        for j in range(i + 1, len(groups_with_signatures)):
            ...
            if similarity >= merge_threshold:
                logger.info(f"Potential Merge Found: Group {group_a_id} ('{group_a_dict.get('group_label')}') and Group {group_b_id} ('{group_b_dict.get('group_label')}') - Similarity: {similarity:.3f}")
                ...
                try:
                    if llm_response:
                        merge_data = json.loads(llm_response.strip().strip("```json").strip("```"))
                        merged_label = merge_data.get("merged_label", merged_label)
                        merged_description = merge_data.get("merged_description", merged_description)
                        logger.info(f"LLM suggested merge label: '{merged_label}'")
                except Exception as llm_err:
                    logger.warning(f"Could not parse LLM merge suggestion: {llm_err}. Using fallback label/description.")
                ...
                try:
                    ...
                    cursor_merge.execute("DELETE FROM two_phase_article_groups WHERE group_id = ?", (deleted_group_id,))

                    conn_merge.commit()
                    logger.info(f"Successfully merged Group {deleted_group_id} into Group {surviving_group_id}. New Label: '{merged_label}'")
                    merged_pairs_count += 1
                    processed_group_ids.add(group_a_id) # Mark both as processed for this pass
                    processed_group_ids.add(group_b_id)

                except sqlite3.Error as db_err:
                    logger.error(f"Database error during merge of {deleted_group_id} into {surviving_group_id}: {db_err}", exc_info=True)
                    if conn_merge: conn_merge.rollback()
                    errors += 1
                finally:
                    if conn_merge: conn_merge.close()
    ...
    logger.info(f"--- Group Merging Pass Finished in {elapsed:.2f} seconds ({merged_pairs_count} pairs merged, {errors} errors) ---")
