...
try:
    ...
    from article_signature import generate_article_signature
     ...
     logging.error(f"Failed to import generate_article_signature from article_signature.py: {e}", exc_info=True)
...
def get_existing_groups(db_path: str = "db/news.db") -> List[Dict]:
    ...
    try:
        ...
        for row in group_rows:
            group_id, main_topic, sub_topic, group_label, description, consistency_score = row
...
def generate_group_signature(group: Dict, db_path: str = "db/news.db") -> Dict:
    ...
    if not generate_article_signature:
         logger.error("generate_article_signature function not available. Cannot generate group signature.")
         return {}
    ...
    for article_id in article_ids:
         try:
              sig = generate_article_signature(article_id, db_path) # Assumes this now returns date/source
              if sig: # Ensure signature generation was successful
                  article_signatures.append(sig)
              else:
                  logger.warning(f"Signature generation returned empty for article {article_id} in group {group_id}")
         except Exception as e:
              logger.error(f"Error generating signature for article {article_id} in group {group_id}: {e}", exc_info=False)
              continue # Skip this article if signature fails

    if not article_signatures:
         logger.warning(f"No valid article signatures generated for group {group_id}. Returning basic signature.")
         return group_signature
    ...
    num_valid_articles = len(article_signatures) # Use count of successfully generated signatures
    ...
    for sig in article_signatures:
        ...
        for tech in sig.get("technologies", []):
            ...
            if tech_id is not None: all_tech_ids.add(tech_id)
        # Products
        for product in sig.get("products", []):
            product_id = product.get("entity_id")
            if product_id is not None: all_product_ids.add(product_id)
        # Events
        ...
        if pub_date_str:
             try:
                 ...
                 if pd.notna(current_date):
                     if latest_published_date is None or current_date > latest_published_date:
                          latest_published_date = current_date
             except Exception as date_err:
                 logger.debug(f"Could not parse date '{pub_date_str}' for article {sig.get('article_id')}: {date_err}")
        # Source Processing
        ...
        if all_entity_ids:
            ...
            for entity_id, entity_name, entity_type in fetched_entities:
                 ...
                 final_entity_details.append({
                     ...
                     "frequency": frequency, "avg_relevance": avg_relevance
                 })
        # Tech details
        if all_tech_ids:
            ...
            tech_details = [{"entity_id": r[0], "entity_name": r[1]} for r in cursor.fetchall()]
        # Product details
        if all_product_ids:
            placeholders = ",".join("?" for _ in all_product_ids)
            query = f"SELECT entity_id, entity_name FROM entity_profiles WHERE entity_id IN ({placeholders})"
            cursor.execute(query, list(all_product_ids))
            product_details = [{"entity_id": r[0], "entity_name": r[1]} for r in cursor.fetchall()]
    except sqlite3.Error as e:
        logger.error(f"Database error fetching details for group {group_id}: {e}")
    finally:
        if conn: conn.close()

    company_details = [{"company_name": c, "frequency": cnt / num_valid_articles} for c, cnt in company_counts.items()]
    cve_details = [{"cve_id": c, "frequency": cnt / num_valid_articles} for c, cnt in cve_counts.items()]
    event_details = [{"event_name": n, "frequency": cnt / num_valid_articles} for n, cnt in event_counts.items()]

    # --- Step 4: Populate final signature ---
    group_signature["primary_entities"] = sorted(final_entity_details, key=lambda x: (x["frequency"], x["avg_relevance"]), reverse=True)
    group_signature["companies"] = sorted(company_details, key=lambda x: x["frequency"], reverse=True)
    group_signature["cves"] = sorted(cve_details, key=lambda x: x["frequency"], reverse=True)
    group_signature["technologies"] = tech_details
    group_signature["products"] = product_details
    group_signature["events"] = sorted(event_details, key=lambda x: x["frequency"], reverse=True)
...
def calculate_article_to_group_similarity(
    ...
    api_key: Optional[str] = None # Added optional api_key if needed for description similarity
) -> Dict[str, float]:
    """
    ...
    CORE_ENTITY_TYPES = {'product', 'organization', 'technology'} # Types likely to be core topics

    # --- Calculate Base Similarities ---
    # Entity similarity (using avg_relevance and frequency)
    article_entities_dict = {e["entity_id"]: e.get("relevance_score", 0.7)
    ...
    if article_entities_dict and group_signature.get("primary_entities"):
        ...
        for group_entity in group_signature["primary_entities"]:
            ...
            if group_entity_id in article_entities_dict:
                ...
                combined_weight = article_relevance * group_avg_relevance * group_frequency
                entity_score += combined_weight
        results["entity_similarity"] = entity_score / max_possible_score if max_possible_score > 0 else 0.0
    else:
    ...
    group_companies = {comp.get("company_name") for comp in group_signature.get("companies", []) if comp.get("company_name")}
    ...
    group_cves = {cve.get("cve_id") for cve in group_signature.get("cves", []) if cve.get("cve_id")}
    ...
    article_event_names = {e.get("event_name") for e in article_signature.get("events", []) if e.get("event_name")}
    if article_event_names and group_signature.get("events"):
        event_score = 0.0
        max_possible_event_score = 0.0
        group_event_dict = {e["event_name"]: e.get("frequency", 0.0) for e in group_signature["events"] if e.get("event_name")}
        for event_name, group_freq in group_event_dict.items():
            max_possible_event_score += group_freq # Sum of frequencies as normalization factor
            if event_name in article_event_names:
                 event_score += group_freq # Add frequency if event matches
        results["event_similarity"] = event_score / max_possible_event_score if max_possible_event_score > 0 else 0.0
    else:
    ...
    if article_pub_date_str and group_latest_pub_date_str:
        try:
            article_pub_date = pd.to_datetime(article_pub_date_str, utc=True, errors='raise')
            group_latest_pub_date = pd.to_datetime(group_latest_pub_date_str, utc=True, errors='raise')
            ...
            logger.warning(f"Could not parse/compare dates. Art: '{article_pub_date_str}', Grp: '{group_latest_pub_date_str}'. Err: {e}")
    ...
    try:
        ...
        if article_primary_entities:
             sorted_article_entities = sorted(article_primary_entities, key=lambda x: x.get("relevance_score", 0.0), reverse=True)
             if sorted_article_entities: top_article_entity = sorted_article_entities[0]

        top_group_entity = None
        group_primary_entities = group_signature.get("primary_entities", [])
        if group_primary_entities:
             sorted_group_entities = sorted(group_primary_entities, key=lambda x: x.get("frequency", 0.0) * x.get("avg_relevance", 0.0), reverse=True)
             if sorted_group_entities: top_group_entity = sorted_group_entities[0]

        if (top_article_entity and top_group_entity and
            top_article_entity.get("entity_id") == top_group_entity.get("entity_id") and
            top_article_entity.get("entity_type") in CORE_ENTITY_TYPES):
                core_entity_bonus = CORE_ENTITY_BONUS
                logger.debug(f"Core entity bonus applied: +{core_entity_bonus:.3f} (Entity: '{top_article_entity.get('entity_name')}')")
    except Exception as e:
        logger.warning(f"Error calculating core entity bonus: {e}")
    # --- Final Score Calculation ---
    adjusted_score = composite_score + temporal_adjustment + source_bonus + core_entity_bonus
    ...
    logger.debug(f"Art {article_signature.get('article_id')} vs Grp {group_signature.get('group_id')}: Base={composite_score:.3f}, Temp={temporal_adjustment:.3f}, Src={source_bonus:.3f}, Core={core_entity_bonus:.3f} -> Final={final_score:.3f}")
    return results

def add_article_to_group(article_id: int, group_id: int, db_path: str = "db/news.db", cursor=None) -> bool:
    ...
    try:
        cursor.execute("SELECT group_id FROM two_phase_article_group_memberships WHERE article_id = ?", (article_id,))
        existing_group = cursor.fetchone()

        if existing_group:
            if existing_group[0] == group_id:
                logger.info(f"Article {article_id} already in group {group_id}.")
                if conn_managed_here and conn: conn.close()
                return True
            else:
                logger.warning(f"Moving Article {article_id} from group {existing_group[0]} to {group_id}.")
                cursor.execute("DELETE FROM two_phase_article_group_memberships WHERE article_id = ?", (article_id,))

        cursor.execute("INSERT INTO two_phase_article_group_memberships (article_id, group_id) VALUES (?, ?)", (article_id, group_id))
        ...
        logger.error(f"DB Error in add_article_to_group ({article_id} -> {group_id}): {e}", exc_info=True)
        if conn_managed_here and conn: conn.rollback()
        return False
    except Exception as e: # Catch other unexpected errors
        logger.error(f"Unexpected Error in add_article_to_group ({article_id} -> {group_id}): {e}", exc_info=True)
        if conn_managed_here and conn: conn.rollback()
        return False
    finally:
        if conn_managed_here and conn:
...
def main():
    ...
    parser.add_argument("--db-path", type=str, default="db/news.db", help="Path to the database")
    parser.add_argument("--new-article-id", type=int, help="ID of new article to process")
    parser.add_argument("--process-all", action="store_true", help="Process all ungrouped articles")
    args = parser.parse_args()
    
    try:
        if args.new_article_id:
            logger.info(f"Processing article {args.new_article_id} with threshold {args.threshold}")
            result = process_new_article(args.new_article_id, args.threshold, args.db_path)
            ...
            results = process_multiple_articles(ungrouped_articles, args.threshold, args.db_path)
            
            # Summarize results
            total = len(results)
            added_to_existing = sum(1 for r in results.values() if r.get("action") == "added_to_existing_group")
            created_new = sum(1 for r in results.values() if r.get("action") == "created_new_group")
