// src/pages/BusinessFinanceTrade.jsx
import React, { useState, useEffect } from 'react';
import { ChevronDown, ChevronRight, Clock, Loader2, LineChart } from 'lucide-react';
import Card from '../components/card'; // Assuming Card component exists

export default function BusinessFinanceTrade({ darkMode, selectedTimeRange }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`/api/business_finance_trade_groups?hours=${selectedTimeRange}`);
        if (!res.ok) {
        ...
        console.error('Error fetching Business/Finance groups:', error);
    ...
    <>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold flex items-center"> {/* Added flex */}
          <LineChart className="inline-block mr-2 h-6 w-6" />
      ...
      {!isLoading && groups.length === 0 && (
        <div className="text-center py-10">
          <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4 ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
            <LineChart className={`w-8 h-8 ${darkMode ? 'text-gray-300' : 'text-gray-500'}`} />
          </div>
          <h3 className="text-xl font-medium mb-2">No business updates</h3>
          <p className={`text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6`}>
            {selectedTimeRange > 0
              ? `No business articles found in the last ${selectedTimeRange} hour${selectedTimeRange !== 1 ? 's' : ''}.`
